...
final RegExp latexRegExp = RegExp(
  r"<(tex|texLine)>.*?</(tex|texLine)>",
  multiLine: true,
  dotAll: true,
);
final RegExp mentionRegExp = RegExp(
  r"<(floor|hole)Mention>(.*?)</(floor|hole)Mention>",
);
...
String renderText(
  ...
  String formulaPlaceholder,
  String stickerPlaceholder, {
...
class OTTitle extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    OTDivision? division = context.select<ForumProvider, OTDivision?>(
      (value) => value.currentDivision,
    );
      ...
      child: TagContainer(
        ...
        onChoice: (Tag tag, list) {
          if (tag.tagTitle == homepageDivision.name) {
            ...
            context.read<ForumProvider>().currentDivisionId = DivisionId(
...
class ForumSubpage extends PlatformSubpage<ForumSubpage> {
  ...
  Create<List<AppBarButtonItem>> get trailing => (cxt) {
    ...
    void onChangeTimeRange(BuildContext context, ForumTimeRange newTimeRange) {
      context.read<SettingsProvider>().forumTimeRange = newTimeRange;
      RefreshListEvent().fire();
    }
    ...
    PopupMenuOption buildSortOption(SortOrder sortOrder) => PopupMenuOption(
      label: sortOrder.displayTitle(cxt),
      cupertino:
          (context, platform) => CupertinoPopupMenuOptionData(
            isDefaultAction: currentSortOrder == sortOrder,
          ),
      onTap: (_) => onChangeSortOrder(cxt, sortOrder),
    );
    final List<PopupMenuOption> sortOptions =
        SortOrder.values
            .map((sortOrder) => buildSortOption(sortOrder))
            .toList();
    final PopupMenuOption currentSortOption =
        sortOptions[SortOrder.values.indexOf(currentSortOrder)];

    final ForumTimeRange currentTimeRange = cxt
        .select<SettingsProvider, ForumTimeRange>(
          (value) => value.forumTimeRange,
        );
    PopupMenuOption buildTimeRangeOption(ForumTimeRange timeRange) =>
        PopupMenuOption(
          label: timeRange.displayTitle(cxt),
          cupertino:
              (context, platform) => CupertinoPopupMenuOptionData(
                isDefaultAction: currentTimeRange == timeRange,
              ),
          onTap: (_) => onChangeTimeRange(cxt, timeRange),
        );
    final List<PopupMenuOption> timeRangeOptions =
        ForumTimeRange.values
            .map((timeRange) => buildTimeRangeOption(timeRange))
            .toList();
    final PopupMenuOption currentTimeRangeOption =
        timeRangeOptions[ForumTimeRange.values.indexOf(currentTimeRange)];
    ...
    return [
      ...
      AppBarButtonItem(
        ...
        PlatformPopupMenuX(
          options: sortOptions,
          cupertino:
              (context, platform) => CupertinoPopupMenuData(
                cancelButtonData: CupertinoPopupMenuCancelButtonData(
                  child: Text(S.of(context).cancel),
                ),
              ),
          material:
              (context, platform) => MaterialPopupMenuData(
      ...
      AppBarButtonItem(
        currentTimeRange.displayTitle(cxt),
        PlatformPopupMenuX(
          ...
          material:
              (context, platform) => MaterialPopupMenuData(
                initialValue: currentTimeRangeOption,
                itemBuilder:
                    (context) =>
                        timeRangeOptions
                            .map(
                              (option) => CheckedPopupMenuItem<PopupMenuOption>(
                                value: option,
                                checked: option == currentTimeRangeOption,
                                child: Text(option.label ?? ''),
                              ),
                            )
                            .toList(),
      ...
      AppBarButtonItem(
        ...
        Icon(PlatformX.isMaterial(cxt) ? Icons.visibility : CupertinoIcons.eye),
        () {
          if (cxt.read<ForumProvider>().isUserInitialized) {
            smartNavigatorPush(
              cxt,
      ...
      AppBarButtonItem(
        S.of(cxt).favorites,
        Icon(
          PlatformX.isMaterial(cxt) ? Icons.star_outline : CupertinoIcons.star,
        ),
        () {
          if (cxt.read<ForumProvider>().isUserInitialized) {
            smartNavigatorPush(
...
class ForumSubpageState extends PlatformSubpageState<ForumSubpage> {
  ...
  final TimeBasedLoadAdaptLayer<OTHole> adaptLayer = TimeBasedLoadAdaptLayer(
      ...
      case PostsType.FILTER_BY_ME:
        List<OTHole>? loadedPost = await adaptLayer
            .generateReceiver(listViewController, (lastElement) {
              ...
              return ForumRepository.getInstance().loadUserHoles(
                time,
                sortOrder: SortOrder.LAST_CREATED,
              );
            })
            .call(page);
        ...
        loadedPost = loadedPost?.filter(
          (element) =>
              !SettingsProvider.getInstance().hiddenMyPosts.contains(
                element.hole_id,
              ),
        );
        ...
        List<OTHole>? loadedPost = await adaptLayer
            .generateReceiver(listViewController, (lastElement) {
              final SortOrder sortOrder =
                  ...
                  SortOrder.LAST_REPLIED;
              final ForumTimeRange timeRange =
                  context.read<SettingsProvider>().forumTimeRange;
              final DateTime now = DateTime.now();
              DateTime time = DateTime.now();
              double? cursorScore;
              int? cursorId;
              if (lastElement != null) {
                if (sortOrder.usesScoreCursor) {
                  cursorScore = lastElement.sort_score;
                  cursorId = lastElement.hole_id;
                } else {
              ...
              return ForumRepository.getInstance().loadHoles(
                ...
                cursorId: cursorId,
                createdStart: timeRange.createdStart(now),
              );
            })
            .call(page);
    ...
    return Card(
      child: ListTile(
        ...
        onTap:
            () => Noticing.showNotice(
              context,
              S.of(context).silence_detail,
              title: S.of(context).silence_notice,
              useSnackBar: false,
            ),
      ),
    );
  ...
  void initState() {
    ...
    _postSubscription.bindOnlyInvalid(
      Constant.eventBus.on<CreateNewPostEvent>().listen((_) async {
        ...
        final bool success = await OTEditor.createNewPost(
          context,
          currentDivision is DivisionId ? currentDivision.id : null,
          interceptor: (_, PostEditorText? text) async {
            if (text?.tags.isEmpty ?? true) {
              return await Noticing.showConfirmationDialog(
                    context,
                    S.of(context).post_has_no_tags,
              ...
              PlatformIconButton(
                ...
                onPressed: () async {
                  if (await Noticing.showConfirmationDialog(
                        context,
                        S.of(context).restore_hidden_confirm,
    ...
    return Container(
      ...
      child: RefreshIndicator(
        ...
        onRefresh: () async {
          ...
          try {
            await PrimaryScrollController.of(context).animateTo(
              0,
  ...
  Widget _buildOTListView(
        ...
        (context) => Container(
          ...
          child: Center(
            child: Text(switch (_postsType) {
              ...
              PostsType.SUBSCRIBED_DISCUSSION => S.of(context).no_subscriptions,
    ...
    fatalErrorBuilder: (_, error) {
      if (error is NotLoginError) {
        ...
        return OTQuizWidget(
          successCallback: () async {
            // Update user data
            await ForumRepository.getInstance().getUserProfile(
              forceUpdate: true,
            );
            refreshList();
          },
      ...
      return ErrorPageWidget.buildWidget(context, error, onTap: refreshSelf);
    },
    ...
    onDismissItem: switch (_postsType) {
      ...
      PostsType.FILTER_BY_ME => (context, index, item) {
        ...
        Noticing.showMaterialNotice(context, S.of(context).hide_post_success);
      },
      _ => null,
    },
    onConfirmDismissItem: switch (_postsType) {
      PostsType.FAVORED_DISCUSSION => (context, index, item) {
        return Noticing.showConfirmationDialog(
          context,
          S.of(context).remove_favorite_hole_confirmation,
          isConfirmDestructive: true,
        );
      },
      PostsType.SUBSCRIBED_DISCUSSION => (context, index, item) {
        return Noticing.showConfirmationDialog(
          context,
          S.of(context).remove_subscribed_hole_confirmation,
          isConfirmDestructive: true,
        );
      },
      PostsType.FILTER_BY_ME => (context, index, item) {
        return Noticing.showConfirmationDialog(
          context,
          S.of(context).hide_post_confirm,
          isConfirmDestructive: true,
        );
      },
      _ => null,
...
class TimeBasedLoadAdaptLayer<T> {
  ...
  DataReceiver<T> generateReceiver(
    PagedListViewController<T> controller,
    ...
    return (int pageIndex) async {
      ...
      if ((controller.length() == 0 && _lastSeenElement == null) ||
        ...
        return receiver.call(
          controller.getElementAt(nextPageEnd - startPage - 1),
        );
      } else {
